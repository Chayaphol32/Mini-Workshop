...
$menuId = parse_id($_GET['id'] ?? null);
if ($menuId === null) {
    http_response_code(400);
    exit('รหัสเมนูไม่ถูกต้อง');
}
$menu = admin_find_menu_item($pdo, $menuId);
if ($menu === null) {
...
$pathPrefix = '../';
$errors = [];
$values = ['name' => (string) $menu['name'], 'description' => (string) $menu['description'], 'price' => (string) $menu['price'], 'status' => (string) $menu['status']];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errors === []) {
        admin_update_menu_item($pdo, $menuId, ['name' => $values['name'], 'description' => $values['description'], 'price' => $price, 'status' => $values['status']]);
        flash('success', 'แก้ไขเมนูเรียบร้อยแล้ว');
...
?>
<section class="page-header page-heading"><div><span class="eyebrow">MENU CATALOG</span><h1>แก้ไขเมนูอาหาร</h1><p class="muted">การแก้ราคาไม่มีผลต่อราคาในออเดอร์เดิม</p></div><div class="page-header-actions"><a class="button button-muted" href="<?= e(app_url('admin/menu.php')) ?>">กลับเมนู</a></div></section>
<section class="card form-card form-shell"><div class="section-heading"><div><span class="eyebrow">EDIT MENU ITEM</span><h2>รายละเอียดเมนู</h2><p class="muted">อัปเดตข้อมูลเมนูและสถานะการขายของร้าน</p></div></div><?php if ($errors !== []): ?><ul class="error-list error-summary" role="alert" aria-label="เกิดข้อผิดพลาด"><?php foreach ($errors as $error): ?><li><?= e($error) ?></li><?php endforeach; ?></ul><?php endif; ?><form method="post" action="<?= e(app_url('admin/menu_edit.php?id=' . $menuId)) ?>"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><div class="form-grid"><div class="field field-full"><label for="name">ชื่อเมนู</label><input id="name" name="name" value="<?= e($values['name']) ?>" maxlength="120" required></div><div class="field field-full"><label for="description">รายละเอียด</label><input id="description" name="description" value="<?= e($values['description']) ?>" maxlength="255"></div><div class="field"><label for="price">ราคา (บาท)</label><input id="price" name="price" value="<?= e($values['price']) ?>" inputmode="decimal" pattern="[0-9]+(\.[0-9]{1,2})?" required></div><div class="field"><label for="status">สถานะ</label><select id="status" name="status"><option value="active" <?= $values['status'] === 'active' ? 'selected' : '' ?>>เปิดขาย</option><option value="inactive" <?= $values['status'] === 'inactive' ? 'selected' : '' ?>>ปิดขาย</option></select></div></div><div class="actions form-actions"><button class="button button-primary" type="submit">บันทึกการแก้ไข</button><a class="button button-muted" href="<?= e(app_url('admin/menu.php')) ?>">ยกเลิก</a></div></form></section>
